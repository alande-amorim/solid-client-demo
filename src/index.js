import {
  ...
  const buttonReadFile = document.querySelector("#btnReadFile");
  const filecontents = document.querySelector("#filecontents");
  const btnWriteFile = document.querySelector("#btnWriteFile");
  
  // 1a. Start Login Process. Call login() function.
  function loginToInruptDotCom() {
    return login({
      oidcIssuer: document.querySelector('#oidIssuer').value,
  ...
  async function writeFile() {
    const fileUrl = document.getElementById("writeFileUrl").value;
    const contents = document.getElementById("write-contents").value;
  ...
  buttonReadFile.onclick = function() {  
    readFile();
  };
  
  btnWriteFile.onclick = function() {
    writeFile();
  }